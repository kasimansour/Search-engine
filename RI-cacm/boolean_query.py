    ...
    while True:
        ...
        while query != "":
            if "(" not in query:  # La requête est uniquement constituée de ET
                (query, relevant_docs) = query_without_or(query, cacm_index, doc_id_collection, relevant_docs)
...
def query_with_or(query, index, doc_id_collection, relevant_docs):
    ...
    query = query.replace(query[opening_parenthesis:closing_parenthesis + 1], "")
    # On efface la parenthèse
...
def query_without_or(query, index, doc_id_collection, relevant_docs):
    ...
    try:
        if "NON" not in term:  # Il n'y a pas de NON avant le mot
            ...
            if relevant_docs == []:
                relevant_docs = list(
                    set(doc_id_collection) - set(index[term].keys())) if term in index else list(
                    doc_id_collection)
                #  Si le mot entré n'est pas dans la collection on envoie tous les docID
            else:
                if term in index:
                    # On vérifie si le mot entré est bien dans la collection, sinon on ne le traite pas
                    negation = list(set(doc_id_collection) - set(index[term].keys()))
